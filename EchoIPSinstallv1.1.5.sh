...
LOG_FILE="/home/${USER}/EchoIDSinstall.log"
CHECKPOINT_FILE="/home/${USER}/checkpoint"
TEMP_VAR_FILE="/home/${USER}/TempVariables" 
...
modify_ip() {
    ...
    local base_ip=$(echo "$IP_ADDRESS" | awk -F'.' '{print $1"."$2"."$3}')
    
    local new_octet=$((last_octet + 50))

    # If new_octet exceeds 255, subtract 50 instead
    if [ "$new_octet" -gt 255 ]; then
        new_octet=$((last_octet - 50))
    fi

    # Concatenate the base IP with the modified last octet
    local new_ip="${base_ip}.${new_octet}"
    DHCP_LEASE_END="$new_ip"
    echo "$new_ip"
}
...
net.ipv4.conf.${LOCAL_IF}.accept_redirects = 0
net.ipv4.conf.all.send_redirects = 0
net.ipv4.conf.default.send_redirects = 0
net.ipv4.conf.${LOCAL_IF}.send_redirects = 0
...
## Step : Install and configure Snort
            ...
            update_checkpoint "Set IPtables rules"
            ;;
        "Set IPtables rules")

            log "Modifying Snort configuration..."
            sudo sed -i "s|^ipvar HOME_NET .*|ipvar HOME_NET "${GATEWAY}"|" "$SNORT_CONF"
            sudo sed -i "s|^#ipvar EXTERNAL_NET !"\$HOME_NET"|ipvar EXTERNAL_NET !"\$HOME_NET"|" "$SNORT_CONF"
            sudo sed -i "s|^ipvar EXTERNAL_NET any|#ipvar EXTERNAL_NET any|" "$SNORT_CONF"
            ...
            sudo sed -i "s|^include \$RULE_PATH/.*\.rules|s|^|# |" $SNORT_CONF
            echo "include /etc/snort/rules/local.rules" | sudo tee -a $SNORT_CONF
...
## Step : Download and configure PulledPork
            ...
            sudo chmod +x /home/${USER}/pulledpork/pulledpork.pl
            sudo cp /home/${USER}/pulledpork/pulledpork.pl /usr/local/bin
            sudo cp /home/${USER}/pulledpork/etc/*.conf /etc/snort
        ...
        "Downloading PulledPork")
            ...
            sudo sed -i "s|^rule_path=.*|rule_path=${RULE_PATH}snort.rules|" /etc/snort/pulledpork.conf
...
## Step : Pull the pork and taste
            ...
            update_checkpoint "Testing Snort"
            ;;
        "Testing Snort")
